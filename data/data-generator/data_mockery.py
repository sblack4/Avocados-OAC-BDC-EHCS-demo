...
# class RowWriter:
#     """Writes rows to stdout if DEBUG else to csv
#     """
#     def __init__(self, filename="data-{}.csv".format(date.today())):
#         self.filename = filename
#         self.filehandle = ""
#         if DEBUG:
#             self.filehandle = open(filename, "w+")
#     def row_to_stdout(self,row):
#         print(row)
...
class TextGenerator:
    ...
    def __init__(self, fake, date_generator):
        ...
        positive_text = ["good", "happy", "great", "best"]
        self.positivity_generator = lambda: " ".join([generator_factory(positive_text)() for x in range(3)])
    
    def positive_words(self):
        for_month = self.date_gen().month
        positive_tweet = randrange(0, 100) <= self.positivity_map[for_month] * 1
        if positive_tweet:
